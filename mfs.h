...
typedef struct ParseDir {
	directoryEntry *dir; // (n - 1) element Directory in memory
	int exists;			 // 0 if last element exists 
                         // 1 if path is valid but file doesnt exist
                         // 2 if invalid path
	int location;		 // index location of the last element Directory if it exists
...
typedef struct
	...
	directoryEntry* dir;
	struct fs_diriteminfo* iteminfo;
	} fdDir;
